...
export default async function SellerLayout({ children }: { children: React.ReactNode }) {
  ...
  // Rol distinto de seller: fuera del portal (middleware ya redirige tambien)
  if (su.role !== 'seller') redirect('/')

  // Sesion sin user id: renderizar error page en vez de redirect (evita loop
  // con el middleware que empuja a seller a /seller)
  if (!su.id) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg p-4">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-display font-semibold mb-2">Sesión incompleta</h1>
          <p className="text-sm text-text-secondary mb-6">
            La sesión de tu navegador está en un estado inconsistente. Cerrá sesión y volvé a entrar.
          </p>
